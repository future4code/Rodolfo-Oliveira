 ...
 */
   ...
   if(maoUsuario.texto === ["A", "A"]){confirm(
      `Suas cartas são ${maoUsuario}, por as cartas serão sorteadas novamente` +
      "\n"+ 
      "Entendido?"
   )}else if(maoComputador.texto === ["A", "A"]){confirm(
      `As cartas do computador são ${maoComputador}, por isso as caras serão sorteadas novamente` +
      "\n"+ 
      "Entendido?"
   )}
    ...
    if((valorUsuarioTotal > valorComputadorTotal) <= 21) {
       console.log ("O Usuario venceu!")}else if((valorUsuarioTotal < valorComputador) <= 21){
